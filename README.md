...
Download the appropriate binary from the [latest release](https://github.com/cwpearson/bibliography-checker/releases/latest)
...
```bash
xattr -d com.apple.quarantine bibliography-checker-darwin-arm64
chmod +x bibliography-checker-darwin-arm64
./bibliography-checker-darwin-arm64
```
...
```bash
chmod +x bibliography-checker-linux-amd64
./bibliography-checker-linux-amd64
```
...
```bash
go run main.go --shirty-api-key sk-I9x3ZXSWaI3vsUmXEg6Mfg test/20231113_siefert_pmbs.pdf 
```
```
=== Entry 1 ===
...
```bash
go run main.go --shirty-api-key sk-I9x3ZXSWaI3vsUmXEg6Mfg test/20231113_siefert_pmbs.pdf --entry 14
```
```
=== Entry 14 ===
...
* Before open-sourcing
  * Bib-only version of pap293s1 and overwrite history
  * Bib-only version of pap942s2 and overwrite history
  * "bibliography-check" -> "bibcheck"
